...
drb_server_address = $drb_server_address ? "druby://#{$drb_server_address}" : $currentCall.getHeader('x-rayo-drb-address')

log "====>Connecting to the Ozone Test Server @ #{drb_server_address}<===="

def call_rayo
  call "sip:#{$ozone_testing_server.config['rayo_server']['sip_uri']}"
end
